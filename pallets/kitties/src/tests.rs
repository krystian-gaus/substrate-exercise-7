...
#[test]
fn can_increase_kitty_difficulty_multiplier() {
    new_test_ext().execute_with(|| {
        // use three kittens to ensure that only the difficulty multiplier
        // of the kittens passed to auto_breed is increased

        assert_ok!(KittiesModule::create(Origin::signed(100)));
        assert_ok!(KittiesModule::create(Origin::signed(101)));
        assert_ok!(KittiesModule::create(Origin::signed(101)));

        assert_eq!(KittiesModule::kitty_difficulty_multiplier(0), 0);
        assert_eq!(KittiesModule::kitty_difficulty_multiplier(1), 0);
        ...
        assert_ok!(KittiesModule::auto_breed(Origin::none(), 0, 2, 0, 0));

        assert_eq!(KittiesModule::kitty_difficulty_multiplier(0), 2);
        assert_eq!(KittiesModule::kitty_difficulty_multiplier(1), 1);
        assert_eq!(KittiesModule::kitty_difficulty_multiplier(2), 1);
    });
}

#[test]
fn can_validate_solution() {
    new_test_ext().execute_with(|| {
        // the increase of difficulty used in validate_solution is only checked indirectly
        ...
        let kitty_id_1 = 0;
        let kitty_id_2 = 1;
        // nonce and solution were chosen in such a way that you can see an output change
        // in validate_solution after the corresponding kitty difficulty multipliers have been increased 
        let nonce = 20;
        let solution = 0;

        assert_eq!(KittiesModule::validate_solution(kitty_id_1, kitty_id_2, nonce, solution), true);
        assert_ok!(KittiesModule::auto_breed(Origin::none(), kitty_id_1, kitty_id_2, nonce, solution));
        assert_eq!(KittiesModule::validate_solution(kitty_id_1, kitty_id_2, nonce, solution), false);
    });
}